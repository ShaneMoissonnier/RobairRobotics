...
//then we end the current cluster with the previous hit and start a new cluster with the current hit
    ...
    for( int loop=1; loop<nb_beams; loop++ )//loop over all the hits
    	...
    	//euclidian DISTANCE between the previous hit and the current one is higher than "cluster_threshold"
        ...
        {//the previous hit is the end of the current cluster
            ...
            middle.x = (current_scan[end].x + current_scan[start].x)/2.0; 
            middle.y = (current_scan[end].y + current_scan[start].y)/2.0; // compute the middle of the cluster
